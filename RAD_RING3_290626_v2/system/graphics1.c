...
// Altere sua graphics_init para tentar recuperar o erro
void graphics_init(void) {
    ...
    // CORREÇÃO: Se falhar em 1080p, não mate o SO, force uma resolução menor!
    if (!ram_buffer) {
        // Tenta alocar para uma resolução padrão menor de segurança (Ex: 1024x768)
        screen_w = 1024;
        screen_h = 768;
        screen_pitch = 1024 * bpp_bytes;
        buffer_size = (size_t)screen_pitch * screen_h;
        
        ram_buffer = (uint8_t*)malloc(buffer_size);
        if (!ram_buffer) {
            sys_exit(); // Se nem em 1024x768 alocar, aí sim paramos.
        }
    }
